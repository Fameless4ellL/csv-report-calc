...
Приложение читает CSV-файлы биржевых данных из указанной директории, вычисляет медиану цен по мере поступления данных и сохраняет в выходной файл.
...
```bash
# С указанием конфига
./build/csv_report_calc --config config.toml

# Или краткая форма
./build/csv_report_calc --cfg config.toml

# Без аргументов — ищет config.toml рядом с исполняемым файлом
./build/csv_report_calc
```
